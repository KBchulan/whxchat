...
unsigned char ToHex(unsigned char x)
{
    ...
    // oss << std::hex << std::uppercase << static_cast<int>(x);
    // return oss.str();
}
...
void HttpConnection::HandleReq()
{
    ...
    {
        bool success = LogicSystem::GetInstance()->HandlePost(static_cast<std::string>(_request.target()), shared_from_this());
        if (!success)
        {
            _response.result(http::status::not_found);
            _response.set(http::field::content_type, "text/plain");
...
{
    // 提取url
    auto url = static_cast<std::string>(_request.target());
    // 查询'?'开始的位置
